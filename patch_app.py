...
target = """window.openChildCampaignModal = function() {
...
new_code = """window.openChildCampaignModal = async function() {
    const aw = window.activeWallet || {};
    const ud = window.currentUserData || {};
    const allocated = Number(aw.allocated_credits || ud.allocated_credits || 0) || 0;
    const consumed  = Number(aw.consumed_credits  || ud.consumed_credits  || 0) || 0;
...
if "window.openChildCampaignModal = function() {" in text:
    text = text.replace(target, new_code)
    with open(path, 'w', encoding='utf-8') as f:
        f.write(text)
    print("Successfully patched app.js modal handler")
else:
