...
def is_v27_orchestrator_enabled(
    ...
    *,
    ...
    Precedence:
      1. campaign.flags.v27_intelligence_orchestrator (bool) if present
      2. campaign.v27_intelligence_orchestrator if present
      3. env V27_INTELLIGENCE_ORCHESTRATOR (default false)
    """
    if campaign and isinstance(campaign, Mapping):
        flags = campaign.get("flags") or campaign.get("feature_flags") or {}
        if isinstance(flags, Mapping) and "v27_intelligence_orchestrator" in flags:
            return bool(flags.get("v27_intelligence_orchestrator"))
        if "v27_intelligence_orchestrator" in campaign:
            return bool(campaign.get("v27_intelligence_orchestrator"))
    source = env if env is not None else os.environ
    raw = str(source.get("V27_INTELLIGENCE_ORCHESTRATOR", "false") or "false").strip().lower()
